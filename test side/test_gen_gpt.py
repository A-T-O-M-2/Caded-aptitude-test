...
def get_questions_by_subsection(section, subsection):
    ...
    sql = """
        ...
        LEFT JOIN options o ON q.question_id = o.question_id
        LEFT JOIN option_images oi ON o.option_id = oi.option_id
        WHERE q.section = %s AND q.subsection = %s AND q.passage_id IS NULL
    ...
    # print(f"results : {results}") -- working
    questions = []
    current_question = None
...
#         if section == 'english':
#             # Get all passage-based questions for the English section
#             print("I am English")
#             passages_with_questions = get_passages_with_questions()
#             sections_with_questions[section] = passages_with_questions
#         else:
#             # Get individual questions for other sections
#             print(f"section : {section}")
#             section_questions = get_random_questions(section, num_questions)
#             sections_with_questions[section] = section_questions
...
def generate_test(num_questions):
    ...
    for section in sections:
        if section == 'english':
            passages_with_questions = get_passages_with_questions()
            sections_with_question_ids[section] = [passage['questions'][0]['id'] for passage in passages_with_questions]
        else:
            section_questions = get_random_questions(section, num_questions)
...
def get_passages_with_questions_by_ids(question_ids):
    passages_with_questions = []

    for question_id in question_ids:
        # Get the passage ID for the current question
        sql = "SELECT passage_id FROM questions WHERE question_id = %s"
        values = (question_id,)
        cursor.execute(sql, values)
        result = cursor.fetchone()
        ...
        if result:
            ...
            # Check if the passage is already in the list
            ...
            if not passage_exists:
                ...
                if result:
                    ...
                    passages_with_questions.append(passage)
            ...
            # Add the question to the passage
            for passage in passages_with_questions:
                if passage['passage_id'] == passage_id:
...
def index():
    if request.method == 'POST':
        ...
        if validate_credentials(login, password):
            ...
            test_question_ids = generate_test(num_questions=5)  # Fixed number of questions for all sections
...
section_scores = defaultdict(lambda: {"correct": 0, "total": 10})
...
def test():
    ...
    for section, question_ids in test_question_ids.items():
        print(f"section : {section}")
        print(f"question id: {question_ids}")
        if section == 'english':
            test_questions[section] = get_passages_with_questions_by_ids(question_ids)
        else:
            test_questions[section] = get_questions_by_ids(question_ids)
    time_remaining = 5400
    ...
    if request.method == 'POST':
        ...
        # session['tab_switch_counter'] = int(request.form.get('tab_switch_counter', 0))
