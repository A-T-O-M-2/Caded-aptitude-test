...
def get_questions_by_subsection(section, subsection):
    # Get all passages for the given section and subsection
    sql = """
    ...
    for passage in passages:
        ...
        sql = """
            ...
            WHERE q.passage_id = %s AND q.section = %s AND q.subsection = %s
            ORDER BY q.question_id, o.option_id
        """
        values = (passage_id, section, subsection)
        cursor.execute(sql, values)
...
def generate_test(num_questions):
    ...
    for section in sections:
        ...
        for subsection in subsections:
            passages_with_questions = get_questions_by_subsection(section, subsection)
            # num_passages_to_select = int(num_questions / len(subsections))
            # num_passages_to_select = int(num_questions / len(subsections))
            # selected_passages = random.sample(passages_with_questions, num_passages_to_select)
            # section_questions.extend(selected_passages)
...
def index():
    if request.method == 'POST':
        ...
        if validate_credentials(login, password):
            ...
            test_questions = generate_test(num_questions=1)  # Fixed number of questions for all sections
...
section_scores = defaultdict(lambda: {"correct": 0, "total": 1})
